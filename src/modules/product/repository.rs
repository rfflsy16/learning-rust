...
use sqlx::{
    query, query_as, FromRow, postgres::PgRow, Row,
    QueryBuilder,
};
use uuid::Uuid;
...
impl<'r> FromRow<'r, PgRow> for Product {
    fn from_row(row: &'r PgRow) -> Result<Self, sqlx::Error> {
        ...
        let price = f64::from_str(&price_decimal.to_string())
            .unwrap_or_else(|e| {
...
impl ProductRepository {
    ...
    pub async fn delete(&self, id: Uuid) -> Result<(), ApiError> {
        ...
        if result.rows_affected() == 0 {
            return Err(ApiError::NotFound(format!("Produk dengan ID {} tidak ditemukan", id)));
        }
    ...
    async fn get_current_product<'a>(
    ...
    fn apply_filters<'a>(query_builder: &mut QueryBuilder<'a, sqlx::Postgres>, filter: &'a ProductFilter) {
